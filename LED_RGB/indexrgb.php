...
<head>
    ...
    <script>
        ...
        // Fonction pour modifier la couleur de la LED
...
if (isset($_GET['action'])) {
    ...
    if ($action == 'setColor') {
        ...
        $command = 'sudo python3 /var/www/html/led_set_color.py ' . $red . ' ' . $green . ' ' . $blue;
        $output = shell_exec($command);
    } 
    elseif ($action == 'off') {
        // Exécuter le script Python pour éteindre la lumière et nettoyer les broches GPIO
        $command = 'sudo /python3 /var/www/html/ledrgb_off.py';
        $output = shell_exec($command);
    } else {
